...
\begin{formula}
    ...
    \[\dfrac{p}{T}=C_3\]
\end{formula}\par
这三个定律都是在压强不太大（相对一个标准大气压$101.325\ \mathrm{kPa}$）、温度不太低（相对室温$298\ \mathrm{K}$）的条件下总结出来的。当压强很大、温度很低时，由上述规律计算的结果与实际测量结果有很大的差别。\par
...
\begin{derivation}
    ...
    \[p_iV=n_iRT\Longleftrightarrow p_i=\dfrac{n_iRT}{V}\]
    代入$p=\sum_{i}p_i$中得：
    \[p=\sum_{i}\dfrac{n_iRT}{V}=\dfrac{RT}{V}\sum_{i}n_i\]
    由于总物质的量$\displaystyle n=\sum_{i}n_i$，所以：
    \[p=\dfrac{nRT}{V}\]
    所以：
    ...
    \[p_i=py_i\]
\end{derivation}\par
道尔顿分压定律告诉我们气体$i$的分压和它的摩尔分数成正比，这样我们就可以通过摩尔分数计算分压，在\autoref{2.7}中计算气相反应的平衡常数时非常有用。
