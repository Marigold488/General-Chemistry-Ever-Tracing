...
% 最后还有一些常规内容需要提醒：
% 段落用\par，不要\\强制换行
% 数值和单位之间要空格，单位需要正体书写，基本格式为：$数值\ \mathrm{单位}$。
% 变量本身用斜体书写，如果变量上下标是变量（比如E_n）那么用斜体，如果变量上下标只是符号，用正体书写。碰到自己没写过的单位可以问问AI
% 分数看情况，一般情况用\dfrac，如果实在太大了就用\frac

这种是行内公式$1.14\ \mathrm{mol}$\par
这种是单独一行居中的公式
